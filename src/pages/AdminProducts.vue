      ...
      <div class="flex flex-col gap-6">
          ...
          <table class="min-w-full text-sm">
            ...
            <tbody>
              <tr v-for="(product, index) in filteredProducts" :key="product.id" class="border-t">
                ...
                <td class="p-3">{{ getCategoryName(product) }}</td>
        ...
        <div v-if="showForm" class="rounded-3xl border bg-white p-6 shadow">
          ...
          <form @submit.prevent="saveProduct" class="mt-6 space-y-6">
            ...
            <div class="space-y-3">
              <div v-for="(feature, index) in form.features" :key="index" class="flex flex-col gap-2 md:flex-row md:items-center">
                <input
            ...
            <div class="flex flex-wrap gap-3">
              <button type="submit" class="rounded-full bg-emerald-600 px-6 py-3 text-white hover:bg-emerald-700">
                {{ isEditing ? 'حفظ التعديلات' : 'حفظ المنتج' }}
              </button>
              <button type="button" @click="resetForm()" class="rounded-full bg-slate-100 px-6 py-3 text-slate-700 hover:bg-slate-200">
                إعادة تعيين
              </button>
            </div>
          </form>
...
function toggleVisibility(product) {
  ...
  if (product.hidden) {
    ...
    showToast('تم إظهار المنتج بنجاح 🎉');
  }
}
